...
DEFAULT_DB_PATH = PROJECT_ROOT / "loc_metadata.db"
DEFAULT_STATE_PATH = PROJECT_ROOT / "filter_state.db"
...
def resolve_imageset_dir(imageset_folder: str) -> Path:
    return (images_base_dir() / imageset_folder).resolve()


def find_master_tif(imageset_folder: str) -> Optional[Path]:
    imageset_dir = resolve_imageset_dir(imageset_folder)
    if not imageset_dir.exists() or not imageset_dir.is_dir():
        return None

    candidates = [
        child for child in imageset_dir.iterdir()
        if child.is_file() and child.suffix.lower() in {".tif", ".tiff"}
    ]
    if not candidates:
        return None

    return sorted(candidates, key=lambda path: (not path.stem.lower().endswith("u"), path.name.lower()))[0]


def master_image_url(imageset_folder: str) -> str:
    master_path = find_master_tif(imageset_folder)
    if master_path is None:
        return ""
    return url_for("api_master_image", imageset_folder=imageset_folder)
...
@app.route("/api/master-image", methods=["GET"])
def api_master_image() -> Any:
    imageset_folder = request.args.get("imageset_folder", "").strip()
    if not imageset_folder:
        abort(400, description="imageset_folder is required")

    images_base = images_base_dir()
    imageset_dir = resolve_imageset_dir(imageset_folder)
    try:
        imageset_dir.relative_to(images_base)
    except ValueError:
        abort(403)
    ...
    try:
        ...
        return send_file(master_path, mimetype="image/tiff", max_age=3600)
    ...
    return send_file(
        buffer,
        mimetype="image/jpeg",
        download_name=f"{master_path.stem}.jpg",
        max_age=3600,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Network-accessible filter manager for Gallery Manager metadata")
