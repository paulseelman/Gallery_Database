...
const filterApiBase = String(config.filterApiBase || "").replace(/\/$/, "");
...
function resolveImageUrl(url) {
  ...
  if (url.startsWith("/") && filterApiBase) {
    return `${filterApiBase}${url}`;
  }
  return url;
}
...
function renderItem(item) {
  ...
  const imageUrl = resolveImageUrl(item.master_image_url || item.thumbnail_url);
...
async function pollSelection() {
  try {
    ...
    const source = data.source || "remote";
